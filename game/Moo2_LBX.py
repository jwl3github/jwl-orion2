...
def bitarray(bytes_array):
    ...
    return ba
# ------------------------------------------------------------------------------
def compose_int(b0, b1 = 0, b2 = 0, b3 = 0):
...
def read_char(data, offset):
    return ord(data[offset])
# ------------------------------------------------------------------------------
def read_unsigned_short_int(data, offset):
...
def read_int(data, offset):
    """returns 4-byte signed integer on given offset"""
    i = ord(data[offset]) + (ord(data[offset+1]) << 8) + (ord(data[offset+2]) << 16) + (ord(data[offset+3]) << 24)
    if i > 0x7fffffff:
        i -= 0x100000000
    return i
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
def read_struct(data, struct_offset, struct_table):
    """Read entire struct based on fixed table format [ ['name', 'type_abbr <A|c|bB|sS|iI>', field_offset, (len|count)], ... ]"""
    # Note: A   = ASCII String, and requires the (len) to be filled.
    #       c   = char (synonymous with unsigned byte)
    #       bB  = signed byte        / unsigned byte
    #       sS  = signed short int   / unsigned short int
    #       iI  = signed int         / unsigned int
    #       if (count) supplied for non-'A' then an array of indicated items is produced
#
# JWL Note: Some fields used 'compose_int()' even though it appears regular short read would work.
#           Is this really necessary to distinguish?  May need to add a column to the table...
    struct = {}
    for field in struct_table:
        name, type_abbr, field_offset = field[0:2]
        abs_offset = struct_offset + field_offset
        count      = field[3] if field.__len__() > 2 else 1
        value      = []
        for i in range(count):
            if type_abbr == 'A' or type_abbr == 'a':
                value.append(read_string(data, abs_offset, count))
                count = 1
                break # Do not loop for strings.
            elif type_abbr == 'b':
                value.append(read_byte(data, abs_offset))
                abs_offset += 1
            elif type_abbr == 'B':
                value.append(read_unsigned_byte(data, abs_offset))
                abs_offset += 1
            elif type_abbr == 'c':
                value.append(read_char(data, abs_offset))
                abs_offset += 1
            elif type_abbr == 's':
                value.append(read_short_int(data, abs_offset))
                abs_offset += 2
            elif type_abbr == 'S':
                value.append(read_unsigned_short_int(data, abs_offset))
                abs_offset += 2
            elif type_abbr == 'i':
                value.append(read_int(data, abs_offset))
                abs_offset += 4
            elif type_abbr == 'I':
                value.append(read_unsigned_int(data, abs_offset))
                abs_offset += 4
            else:
                print 'ERROR: Moo2_LBX.read_struct() -- type_abbr unknown <%s>' % type_abbr
                break
        if count > 1:
            struct[name] = value
        else:
